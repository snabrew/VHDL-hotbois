...
shared variable int : integer := 123;
...
if falling_edge(subclk) and unsigned(x) >= 5 then 
    x := std_logic_vector((unsigned(x) + 3));
end if;

y := work(16 downto 13);
if falling_edge(subclk) and unsigned(y) >= 5 then 
    y := std_logic_vector((unsigned(y) + 3));
end if;

z := work(12 downto 9);
if falling_edge(subclk) and unsigned(z) >= 5 then 
...
if rising_edge(subclk) and count = 9 and start = '0'   then
...
disp(15 downto 12) <= "0011";
disp(11 downto 8)  <= "0000";
disp(7 downto 4)   <= y;
disp(3 downto 0)   <= "0001";
